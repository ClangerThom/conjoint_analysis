...
Further documentation, the data itself and any outputs of this script such as tables and plots can be acessed from an accompanying Onedrive with the author's permission.
...
Finally, I load in raw data from my Onedrive. The file path is managed by the config, add yours in that file.
...
Finally I export the created plot to my Onedrive.
...
And finally again exporting the output to my Onedrive.
...
The code chunk below creates a standard APA table as a word document exported to my Onedrive.
...
m_interaction_woman_plotting |> 
  ...
  labs(
    x = "Average change on an 11 point scale in comparison to\nreference category",
    y = NULL,
    #title = "Perceptions of parental readiness by gender\n(interaction of vignette gender and components)",
...
In this block I export the result to my Onedrive.
...
## AMCE plot for Q1 interaction between components and income (excluding sex)
...
ggsave("q1_income_interaction_AMCE.png", bg = "white",
       width = 15, height = 17.8, units = "cm",
       path = output_plots)
```
